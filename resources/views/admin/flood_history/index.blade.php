    ...
    <div class="max-w-7xl mx-auto px-6 mt-10">
        <div class="bg-white bg-opacity-80 backdrop-blur-md rounded-2xl shadow-lg p-6 text-gray-800"> {{-- Main container matches admin dashboard style --}}
            ...
            @forelse ($floods as $flood)
                <div class="bg-white shadow-md rounded-xl overflow-hidden mb-5 p-5 flex flex-col md:flex-row items-stretch gap-5 border-l-4 border-[#FFA404] hover:shadow-lg transition transform hover:-translate-y-1"> {{-- Orange left border, subtle lift on hover --}}
                    ...
                    <div class="flex-shrink-0 w-full md:w-56 h-36 rounded-lg overflow-hidden shadow-inner"> {{-- Slightly larger image container --}}
                        ...
                        @else
                            <img src="{{ asset('images/placeholder-flood.jpg') }}" alt="Tidak ada gambar"
                                class="w-full h-full object-cover bg-gray-200 flex items-center justify-center text-gray-500"> {{-- Placeholder for missing image --}}
                        @endif
                    </div>
                    ...
                    <div class="flex-1 flex flex-col justify-between">
                        ...
                        <div class="flex gap-3 mt-auto pt-3 border-t border-gray-100"> {{-- Actions always at bottom, separated by a thin border --}}
                            ...
                            <button type="button" class="bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 flex items-center transition shadow-sm text-sm font-semibold"
                                data-modal-target="delete-modal-{{ $flood->id }}" data-modal-toggle="delete-modal-{{ $flood->id }}">
                ...
                {{-- Delete Confirmation Modal (for each flood history item) --}}
                <div id="delete-modal-{{ $flood->id }}" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full bg-gray-900 bg-opacity-50">
                    <div class="relative p-4 w-full max-w-md max-h-full">
                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                            <button type="button" class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="delete-modal-{{ $flood->id }}">
                            ...
                            <div class="p-4 md:p-5 text-center">
                                ...
                                <form action="{{ route('admin.flood_history.destroy', $flood->id) }}" method="POST" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                                ...
                                <button data-modal-hide="delete-modal-{{ $flood->id }}" type="button" class="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">Tidak, batal</button>
                            </div>
                        </div>
                    </div>
                </div>
